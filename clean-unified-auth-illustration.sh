...
delete_sharedflow() {
    local sf_name=$1
    printf "Checking SharedFlow %s\n" "${sf_name}"
    if apigeecli sharedflows get --name "$sf_name" --org "$PROJECT" --token "$TOKEN" --disable-check >/dev/null 2>&1; then
        OUTFILE=$(mktemp /tmp/apigee-samples.apigeecli.out.XXXXXX)
        if apigeecli sharedflows listdeploy --name "$sf_name" --org "$PROJECT" --token "$TOKEN" --disable-check >"$OUTFILE" 2>&1; then
            NUM_DEPLOYS=$(jq -r '.deployments | length' "$OUTFILE")
            if [[ $NUM_DEPLOYS -ne 0 ]]; then
                echo "Undeploying ${sf_name}"
                for ((i = 0; i < NUM_DEPLOYS; i++)); do
                    ENVNAME=$(jq -r ".deployments[$i].environment" "$OUTFILE")
                    REV=$(jq -r ".deployments[$i].revision" "$OUTFILE")
                    apigeecli sharedflows undeploy --name "${sf_name}" --env "$ENVNAME" --rev "$REV" --org "$PROJECT" --token "$TOKEN" --disable-check
                done
            else
                printf "  There are no deployments of %s to remove.\n" "${sf_name}"
            fi
        ...
        echo "Deleting proxy ${sf_name}"
        apigeecli sharedflows delete --name "${sf_name}" --org "$PROJECT" --token "$TOKEN" --disable-check

    else
        printf "  The sharedflow %s does not exist.\n" "${sf_name}"
    fi
}
...
if apigeecli developers get --email "${DEVELOPER_EMAIL}" --org "$PROJECT" --token "$TOKEN" --disable-check >>/dev/null 2>&1; then
    ...
    for appSuffix in "token" "apikey"; do
        delete_app "$DEVELOPER_ID" "${EXAMPLE_NAME}-app-${appSuffix}"
    done
...
delete_product "${PRODUCT_NAME}-1"
delete_product "${PRODUCT_NAME}-2"
...
delete_sharedflow "apigeesample-unified-auth"
